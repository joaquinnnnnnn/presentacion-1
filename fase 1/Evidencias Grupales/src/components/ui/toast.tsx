...
const toastVariants = cva('group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border bg-white p-6 pr-8 shadow-lg transition-all', {
  variants: { variant: { default: 'border bg-white text-black', destructive: 'border-red-500 bg-red-600 text-white' } }, defaultVariants: { variant: 'default' }
});
...
const ToastAction = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Action>, React.ComponentPropsWithoutRef<typeof ToastPrimitives.Action>>(({ className, ...props }, ref) => (
  <ToastPrimitives.Action ref={ref} className={cn('inline-flex h-8 items-center justify-center rounded-md border bg-transparent px-3 text-sm', className)} {...props} />
));
...
const ToastClose = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Close>, React.ComponentPropsWithoutRef<typeof ToastPrimitives.Close>>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close ref={ref} className={cn('absolute right-2 top-2 rounded-md p-1 text-black/50 hover:text-black focus:outline-none', className)} {...props}><X className="h-4 w-4" /></ToastPrimitives.Close>
));
